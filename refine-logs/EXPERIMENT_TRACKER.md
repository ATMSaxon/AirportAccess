...
| R040 | M4 | `python scripts/sample_counterfactuals.py --airport KLAX --n 200000` | ml-engineer | code-ready (waits on M1 ADS-B + M3 runway-config) | `data/processed/KLAX/counterfactuals.parquet` | All 4 modules (`counterfactual.py`, `features.py`, `risk_field.py`, `conformal.py`) + 2 scripts + 7 tests landed. INTERFACES.md published. Smoke on KSYN: 500 segments, 8.8 % conflict rate. |
...
| R042 | M4 | `python scripts/train_risk_field.py --model mlp --airport KLAX --gpu remote` | ml-engineer | code-ready (waits on M1+M3) | `results/risk/KLAX/mlp.json` | Featurize rsync+ssh wired inline in `scripts/train_risk_field.py::_run_remote` (swap to team-lead's `deploy_featurize.sh` once it lands). |
